      ...
      <div className="h-full p-8 md:p-20 flex flex-col relative border-t-[6px] md:border-t-8" style={{ borderTopColor: accentColor }}>
        <header className="mb-8 md:mb-12">
          <h2 className="text-xl md:text-3xl font-black uppercase tracking-tight mb-2 leading-tight" style={{ color: accentColor }}>
            {pageData.title || `Insights Section — ${pageNum - 1}`}
          </h2>
        ...
        <div className="flex-1 grid grid-cols-12 gap-6 md:gap-10">
          <article className="col-span-12 md:col-span-8">
            <p className="text-gray-800 leading-relaxed md:leading-loose text-xs md:text-base whitespace-pre-wrap mb-10">
              {pageData.content || "Your detailed content for this section will appear here."}
            </p>
            ...
            {pageImage && (
              <div className="mt-8">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-[10px] font-black uppercase text-gray-300 tracking-widest">Detail Visualization</span>
                </div>
                <RenderImage meta={pageImage} className="aspect-[16/9] rounded-2xl shadow-lg grayscale hover:grayscale-0 transition-all" />
              </div>
            )}
          </article>
  ...
  const FrontPage = () => {
    ...
    if (templateId === 'classic') return (
      <div className="h-full border-[6px] md:border-[10px] p-6 md:p-10 flex flex-col relative" style={{ borderColor: accentColor }}>
        ...
        {coverImage && <RenderImage meta={coverImage} className="mb-6 md:mb-10 aspect-[21/9] rounded-xl shadow-sm" />}
    ...
    if (templateId === 'magazine') return (
      <div className="h-full flex flex-col relative bg-white">
        <div className="h-[220px] md:h-[400px] w-full relative bg-slate-100">
          {coverImage ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-6 md:p-10">
            <h1 className="text-2xl md:text-5xl font-black text-white uppercase leading-none tracking-tighter">{eventData.title}</h1>
          </div>
        </div>
        <div className="p-6 md:p-10 grid grid-cols-12 gap-6 md:gap-10 flex-1">
          <div className="col-span-12 md:col-span-8 flex flex-col">
            ...
            <div className="flex gap-4 mb-4 md:mb-6">
              <Quote size={30} className="opacity-10 shrink-0" style={{ color: accentColor }} />
              <p className="text-sm md:text-lg font-medium leading-relaxed italic text-gray-600 border-l-2 pl-4" style={{ borderColor: accentColor }}>An innovative gathering defined by excellence.</p>
            </div>
        ...
        <div className="flex-1 p-6 md:p-12 flex flex-col">
          <header className="mb-8 md:mb-12">
            <div className="flex items-center gap-3 md:gap-4 mb-2">
              ...
              <span className="text-[9px] md:text-xs font-black uppercase tracking-widest text-gray-400">Official Entry</span>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 flex-1">
            <div className="space-y-4 md:space-y-8">
              ...
              {coverImage && (
                <RenderImage meta={coverImage} className="aspect-square grayscale rounded-2xl shadow-lg" />
              )}
            </div>
            <div className="flex flex-col">
              <div className="bg-gray-900 text-white p-5 md:p-8 rounded-2xl md:rounded-3xl flex-1 shadow-2xl">
                <h3 className="text-[10px] md:text-xs font-black uppercase tracking-widest mb-4 md:mb-8 text-indigo-400">Key Takeaways</h3>
                <ul className="space-y-4 md:space-y-6">
                  {eventData.highlights.filter(h => h.trim() !== '').map((item, index) => (
                    <li key={index} className="flex flex-col gap-1 border-b border-white/10 pb-3 last:border-0">
                      <span className="text-[8px] md:text-[10px] font-bold opacity-30 uppercase">0{index + 1}</span>
                      <span className="text-[10px] md:text-xs font-bold leading-tight">{item}</span>
                    </li>
                  ))}
                </ul>
              </div>
    ...
    return (
      <div className="h-full bg-white p-8 md:p-20 flex flex-col justify-center items-center text-center">
        <div className="max-w-xl flex flex-col items-center">
          ...
          {coverImage && (
            <RenderImage meta={coverImage} className="w-full grayscale mb-8 md:mb-12 rounded-xl shadow-md aspect-[16/10]" />
          )}
